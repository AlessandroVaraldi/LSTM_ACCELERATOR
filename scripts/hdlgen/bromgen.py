...
# Create VHDL for ROM
with open('brom.vhd', 'w') as f:
    f.write('library IEEE;\n')
    f.write('use IEEE.STD_LOGIC_1164.ALL;\n')
    f.write('use IEEE.NUMERIC_STD.ALL;\n')
    f.write('entity brom is\n')
    f.write('    port (\n')
    f.write('        clk : in STD_LOGIC;\n')
    f.write('        addr : in UNSIGNED(4 downto 0);\n')
    f.write('        data : out STD_LOGIC_VECTOR(31 downto 0)\n')
    f.write('    );\n')
    f.write('end brom;\n')
    f.write('\n')
    f.write('architecture Behavioral of brom is\n')
    f.write('    type rom_type is array (0 to 32) of STD_LOGIC_VECTOR(31 downto 0);\n')
    f.write('    signal rom : rom_type := (\n')
    i = 0
    print(bx.shape[0])
    for xi in range(bx.shape[0]):
        i = i + 1
        f.write(f'        {int_to_hex_complement(round(bx[xi]*2**24))},\n')
    for hi in range(bh.shape[0]):
        i = i + 1
        f.write(f'        {int_to_hex_complement(round(bh[hi]*2**24))},\n')
    for j in range(31-i):
